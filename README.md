...
A fundamental results is that if $P\in E$, $P P_0 \cdots P_k = 0$ and
$P_0,\ldots P_k$ are independent then

$P = \sum_{j = 0}^k P_0 \cdots P_{j-1} P P_{j + 1}\cdots P_k/P_0\cdots P_k$.
...
cone with vertex $P_0$ if and only if all coefficients except the
